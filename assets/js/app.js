...
const App = {
  ...
  // Normalise level JSON so the app can accept both:
  //  - { levels: [...] } (multi-level bundle)
  //  - { id/level, title, modules: [...] } (single-level file)
  _normaliseLevelPayload(payload, fallbackId) {
    if (!payload || typeof payload !== "object") return { levels: [] };

    // Case 1: bundle format
    if (Array.isArray(payload.levels)) {
      return { levels: payload.levels.filter(Boolean) };
    }

    // Case 2: single level format
    if (Array.isArray(payload.modules)) {
      const id = payload.id || payload.level || fallbackId || "a1";
      const title = payload.title || payload.name || id.toUpperCase();
      return { levels: [{ id, title, modules: payload.modules }] };
    }

    // Unknown shape → return empty to avoid crashing
    return { levels: [] };
  },
...
// Boot once DOM is ready (avoids race conditions)
    ...
    if (el) {
      el.innerHTML = `
        <div class="card">
          ...
          <p>Ouvre la console (F12) pour voir le détail, puis corrige le chemin des fichiers JSON/JS si besoin.</p>
          <pre style="white-space:pre-wrap;">${String(err && err.stack ? err.stack : err)}</pre>
        </div>`;
    }
  });
});
