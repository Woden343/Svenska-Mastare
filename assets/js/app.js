...
const App = {
  ...
  async init() {
    ...
    // ===== UI: thème (persistant) + sidebar mobile + nav active =====
    // Thème
    ...
    // Fermer le menu au clic nav (mobile)
    ...
    const setActiveNav = (key) => {
      const map = { home: "nav-home-link", ref: "nav-ref", review: "nav-review", stats: "nav-stats" };
      Object.values(map).forEach(i => document.getElementById(i)?.classList.remove("is-active"));
      document.getElementById(map[key] || map.home)?.classList.add("is-active");
    };

    // Wrap views so nav updates automatically
  ...
  toggleTheme() {
    ...
    try {
      localStorage.setItem("sm_theme", root.classList.contains("dark") ? "dark" : "light");
    } catch {
      // ignore
    }
  },

  // ===== LESSON ORGANISÉ (décomposition + production + prof) =====
  _norm(s) {
  ...
  _explodeLines(lines) {
    const explodeCompactLine = (s) => {
      ...
      // Break before common markers if inline
      t = t.replace(/\s*(SITUATION\s*[—-]\s*)/gi, "\n$1");
      t = t.replace(/\s*(DIALOGUE\b)/gi, "\n$1");
      t = t.replace(/\s*(D[ÉE]COMPOSITION\b)/gi, "\nDÉCOMPOSITION");
      t = t.replace(/\s*(EXPLICATION DU PROF\b)/gi, "\nEXPLICATION DU PROF");
      t = t.replace(/\s*(PRODUCTION\b|PRODUCTION GUID[ÉE]E\b)/gi, "\nPRODUCTION GUIDÉE");
      t = t.replace(/\s*(VOCABULAIRE\b)/gi, "\nVOCABULAIRE");
  ...
  _extractSection(lines, titleNorms) {
    // titleNorms: array of normalized strings like ["dialogue"]
    const n = (s) => this._norm(s);
    let start = -1;
    for (let i=0;i<lines.length;i++){
      const ln = n(lines[i]);
      if (titleNorms.some(t => ln.includes(t))) { start = i; break; }
    }
    if (start === -1) return { found:false, head:"", body:[] };

    // body starts after heading line
    const head = lines[start].replace(/:$/, "").trim();
    const body = [];
    for (let i=start+1;i<lines.length;i++){
      const ln = n(lines[i]);
      // stop at another big section heading
      if (
        ln.includes("decomposition") ||
        ln.includes("explication du prof") ||
        ln.includes("prononciation") ||
        ln.includes("structures") ||
        ln.includes("vocabulaire") ||
        ln.includes("production")
      ) break;
      body.push(lines[i]);
    }
    return { found:true, head, body };
  },
  _parseDialogueTurns(lines) {
    // expects: "A: ..." "🔊 ..." "→ ..." etc
    const turns = [];
    const speakerRe = /^([A-ZÅÄÖ])\s*:\s*(.+)$/;
    let cur = null;

    const push = () => {
      if (!cur) return;
      const sv = String(cur.sv || "").trim();
      if (!sv) { cur = null; return; }
      turns.push({
        speaker: cur.speaker || "",
        sv,
        pron: String(cur.pron || "").trim(),
        fr: String(cur.fr || "").trim()
      });
      cur = null;
    };
    ...
    for (const raw of (lines || [])) {
      ...
      if (sp) {
        push();
        cur = { speaker: sp[1], sv: sp[2].trim(), pron: "", fr: "" };
        continue;
      }
      if (!cur) continue;
      ...
      if (line.startsWith("🔊")) {
        ...
        cur.pron = cur.pron ? (cur.pron + " • " + p) : p;
        continue;
      }
      if (line.startsWith("→")) {
        const f = line.replace(/^→\s*/, "").trim();
        cur.fr = cur.fr ? (cur.fr + " " + f) : f;
        continue;
      }
      // continue Swedish
      cur.sv = (cur.sv + " " + line).trim();
    }
    push();
    return turns;
  },
  renderLessonOrganized(lesson) {
    const lines = this._explodeLines(lesson?.content);
    if (!lines.length) return "";

    // Dialogue
    const secDialogue = this._extractSection(lines, ["dialogue"]);
    const turns = secDialogue.found ? this._parseDialogueTurns(secDialogue.body) : [];

    // Production guidée (si présente)
    const secProd = this._extractSection(lines, ["production"]);
    const prodText = secProd.found ? secProd.body.join(" ").trim() : "";

    // Prof (si présent)
    const secProf = this._extractSection(lines, ["explication du prof"]);
    const profText = secProf.found ? secProf.body.join(" ").trim() : "";

    // Si rien à structurer → fallback renderer existant
    if (!turns.length && !prodText && !profText) {
      return this.renderContent(lesson.content);
    }

    const dialogueGrid = turns.length
      ? `
        <div class="card">
          <div class="section-title">
            <span class="chip"></span>
            <h2>Dialogue — Décomposition</h2>
          </div>
          <div class="pill-grid">
            ${turns.map((t, idx) => `
              <div class="pill-card">
    ...
    const production = prodText
      ? `
        <div class="card production">
          <div class="row between" style="margin-bottom:10px;">
            <div class="row" style="gap:10px;">
              <span class="material-icons-outlined" style="color: var(--primary);">edit_note</span>
              <div>
                <div style="font-weight:900;">Production guidée</div>
                <div class="muted" style="font-size:13px;">Pratique 2–3 minutes</div>
              </div>
            </div>
          </div>
          <p class="muted" style="margin-top:0;">${this.esc(prodText)}</p>
          <div class="hint" style="margin-top:12px;">
            “Jag heter … Jag kommer från … Jag bor i …”
          </div>
        </div>
      `
      : "";
    ...
    const prof = profText
      ? `
        <div class="card">
          <div class="section-title">
            <span class="chip"></span>
            <h2>Explication du Prof</h2>
          </div>
          <div class="prof-grid-2">
            <div class="prof-note">
              <div class="tag"><span class="dot dot-blue"></span>À retenir</div>
              <p>${this.esc(profText)}</p>
            </div>
            <div class="prof-note">
              <div class="tag"><span class="dot dot-amber"></span>Astuce</div>
              <p>Lis la phrase à voix haute 3 fois, puis refais-la sans regarder. Objectif : rythme + fin des mots.</p>
            </div>
          </div>
        </div>
      `
    ...
    return `${dialogueGrid}${production}${prof}`;
  },
  ...
  viewLesson(params) {
    ...
    this.setView(`
      <div class="stack">
        ...
        ${this.renderLessonOrganized(lesson)}
